...
async def update(interaction: discord.Interaction):
    ...
    caller_role_ids = {role.id for role in caller.roles} if isinstance(caller, discord.Member) else set()
    is_owner = interaction.guild and interaction.guild.owner_id == interaction.user.id

    if not is_owner and not caller_role_ids.intersection(UPDATE_ALLOWED_ROLE_IDS):
        await interaction.response.send_message("You don't have permission to use this command.", ephemeral=True)
        return

    await interaction.response.send_message("Pulling latest changes and restarting.", ephemeral=True)
